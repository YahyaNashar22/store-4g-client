...
export default function EditAccessoryForm({id, setEditAccessories, selectedProduct}) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        setLoading(true)
        axios.put(`${backendURL}accessories/edit`, formData).then(()=>setEditAccessories(false))
   ...
   const handleImageSubmit = (e)=>{
        e.preventDefault();
        setLoading(true);
        axios.put(`${backendURL}accessories/editpicture`, {image:image, id:formData.id} , {
            headers:{
    ...
    const handleDelete = (e) =>{
        e.preventDefault();
        setLoading(true);
        axios.delete(`${backendURL}accessories/delete`,{
            data:{
                id:id
            }
        }).then(()=>setLoading(false), setEditAccessories(false)).catch(e=>console.log(e.message))
    }
...
<form onSubmit={handleSubmit} className="flex flex-col gap-5">
    ...
    <button className="btn-primary" type="submit" disabled={loading}>{loading?"wait...":"submit"}</button>
...
<form>
    ...
    <button className="btn-primary" type="button" onClick={handleImageSubmit}>change picture</button>
</form>
</>
  )
}
