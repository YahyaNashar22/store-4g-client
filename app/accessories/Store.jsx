    ...
    const [selectedProduct, setSelectedProduct] = useState();
        ...
        const fetchProducts = () => {
            if(!search || search===''){
                ...
                axios.post(`${backendURL}accessories/search?page=${page}`, {search:search})
                    .then(res=>{setFetchedProducts(res.data.payload); setTotalNB(res.data.count)})
                    .catch(e=>console.log(e.message))
            }
        }
...
/>
        ...
        {
            fetchedProducts ? fetchedProducts.map((product, key)=>{
                return(
                <div key={key} className="max-w-xs min-w-3.5 rounded overflow-hidden shadow-lg">
                    <img className="transition-transform duration-300 transform hover:scale-105 h-64 w-64" loading='lazy' src={`${backendURL}${product.picture}`} alt={product.name} />
                    <div className="px-6 py-4">
                      <h2 className="text-gray-700 font-bold text-xl mb-2">{product.name}</h2>
        ...
        <div className='flex flex-col w-20'>
          {
            page===1 && totalNB>10 &&
            <button type='button' className='btn-primary' onClick={()=>setPage(page+1)}>Next</button>
          }
          {
            page>1 && page<Math.ceil(totalNB/10) &&
            <div className='flex justify-center align-middle gap-5'>
             <button type='button' className='btn-primary' onClick={()=>setPage(page+1)}>Next</button>
            <button type='button' className='btn-primary' onClick={()=>setPage(page-1)}>Previous</button>
            </div>
          }
          {
            page!=1 && page === Math.ceil(totalNB/10) &&
